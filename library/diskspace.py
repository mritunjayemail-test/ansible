...
def diskspace():
    ...
    if os.path.exists(module.params['path']):
        if os.path.isdir(module.params['path']):
            ...
            dump_du_output = subprocess.Popen(
                ["du", "-chd  1", module.params['path']], stdout=subprocess.PIPE).communicate()[0]
            ...
            storage_path = subprocess.Popen(
                ["df", "-P", module.params['path']], stdout=subprocess.PIPE).communicate()[0]
            if module.params['storage']:
                for device_list in storage_path.split("\n"):
                    if len(device_list.split()) > 0:
                        if device_list.split()[0] != "Filesystem":
                            result['device'] = module.params['path'] + \
                                " " + " is reside on " + device_list.split()[0]

        else:
            result['stderr_line'] = module.params['path'] + \
                " " + "is not directory"
            result['failed'] = True
    else:
